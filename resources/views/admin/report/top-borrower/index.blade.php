...
@section('title', 'Top Borrower Report')

@section('content-header')
    <h1>Top Borrower</h1>
    <ol class="breadcrumb">
        <li><p><i class="fa fa-bar-chart"></i> Reports</p></li>
        <li class="active"><a href="{{ route('report.top-borrowers.index') }}">Top Borrower Report</a></li>
    </ol>
@endsection
        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                ...
                <div class="box-header with-border">
                    <h3 class="box-title">Top Borrower Report</h3>
                </div>
                <!-- box-header -->
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table id="topBorrowersTable" class="table table-bordered table-striped table-hover text-center">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody id="topBorrowersTableBody">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#btnGenerate').click(function() {
                ...
                $.ajax({
                    type : 'POST',
                    url : '/report/top-borrowers',
                    data : {'_token' : CSRF_TOKEN, 'from' : from, 'to' : to, 'patron_type_id' : patronTypeId},
                    success : (response) => {
                        if(response.topBorrowers.length > 0) {
                            $('#btnPrint').removeClass('invisible')

                            let topBorrowersTable = ''
                            response.topBorrowers.forEach(patron => {
                                topBorrowersTable += `
                                    <tr>
                                        <td>${patron.patron_no}</td>
                                        <td>${patron.first_name} ${patron.last_name}</td>
                                        <td>${patron.section.name}</td>
                                        <td>${patron.no_of_borrow}</td>
                                    </tr>
                                `
                            })
                            $('#topBorrowersTableBody').html(topBorrowersTable)
                        }else {
                            $('#btnPrint').addClass('invisible')
                            $('#topBorrowersTableBody').html('<tr><td colspan="4"><h5>No Top Borrower Report Found</h5></td></tr>')
                        }
                    },
                    error : (error) => {
                        $('#btnPrint').addClass('invisible')
            ...
            $('#btnPrint').click(function() {
                ...
                let to = $('#to').val()
                let patronTypeId = $('#patronType').val()
                window.open(`/report/top-borrowers/print?from=${from}&to=${to}&patron_type_id=${patronTypeId}`)
            });
        });
        ...
        function clear() {
            $('.text-danger').html('')
            $('#topBorrowersTableBody').html('<tr><td colspan="4"><h5></h5></td></tr>')
        }
    </script>
@endpush
